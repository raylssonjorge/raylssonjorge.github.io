...
def E_f(t, m, u):
    ...
    for j in range(180):
      ...
      CC = C.pos[0] +   np.deg2rad(np.cos(j))/5 #movimenta a carga de forma
                                                #pequena e periodica
      ...
      CC2 = C.pos[1] + np.deg2rad(np.sin(j))/4 # a mesma coisa
        ...
        for alpha in np.linspace(0, 2*np.pi*15/16, 16):
            ...
            for i in range (200):
                ...
                x[i]  = u0[0]*np.cos(t[i])/30 + u0[0]   # faça os pontos da  
#campo  no eixo x se movimentarem na forma de ondulações ao longo do tempo.
                y[i]  = u0[1]*np.sin((x[i] - 0.01*i)/4*np.pi)/4 +u0[1] #faça as linhas
#de campo no eixo y se movimentarem  devido a mudança dos pontos em x[i]
            ...
            plt.plot(x,y)
            plt.savefig(str(j)+".png") # ---------------------_<<<<<<< SALVA CADA CICLO NO DIRETORIO LOCAL.
#Isso é um jeito que eu encontrei para fazer as animações.Ao salvar cada frame basta encontrar
#um software básico para juntar as imagens
